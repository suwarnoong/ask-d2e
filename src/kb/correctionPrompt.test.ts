...
test("fix mode grounds the correction in code, not existing KB wording", () => {
  const prompt = buildCorrectionPrompt("fix", "acme/widgets", "acme-widgets", "how does auth work?", "it uses OAuth", undefined, undefined);
...
test("fix mode instructs an empty changes array if the original answer was correct", () => {
  const prompt = buildCorrectionPrompt("fix", "acme/widgets", "acme-widgets", "q", "a", undefined, undefined);
  assert.ok(/empty changes array/i.test(prompt));
});

test("gap-fill mode includes file-tree/readme scope-bounding content", () => {
  const prompt = buildCorrectionPrompt("gap-fill", "acme/widgets", "acme-widgets", "what about caching?", "", "src/\n  a.ts", "# Widgets\nA repo.");
...
test("both modes instruct the same KbPlan output shape", () => {
  const fixPrompt = buildCorrectionPrompt("fix", "acme/widgets", "acme-widgets", "q", "a", undefined, undefined);
  const gapPrompt = buildCorrectionPrompt("gap-fill", "acme/widgets", "acme-widgets", "q", "", "tree", "readme");
  for (const p of [fixPrompt, gapPrompt]) {
    assert.ok(/summary/.test(p) && /changes/.test(p));
  }
});

test("both modes require every path to be prefixed with repos/<name>/knowledge-base/", () => {
  const fixPrompt = buildCorrectionPrompt("fix", "acme/widgets", "acme-widgets", "q", "a", undefined, undefined);
  const gapPrompt = buildCorrectionPrompt("gap-fill", "acme/widgets", "acme-widgets", "q", "", "tree", "readme");
  for (const p of [fixPrompt, gapPrompt]) {
    assert.ok(p.includes("repos/acme-widgets/knowledge-base/"));
  }
});
